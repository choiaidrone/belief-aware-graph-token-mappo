"""
token_ppo_v3.py  —  Entity Transformer MAPPO (v12 환경 대응, scale-agnostic)
================================================================
기존 mappo_train_step2.py 대비 변경사항:

  [토큰 구조 (v12)]
    Drone  × N_D : 15dim  pos(2)+depth(8)+local_ds(3)+battery(2)  — other_vec 제거
    Zone   × 16  : 10dim
    ActiveTrack × 12: 6dim
    Global × 1   : 11dim
    N_D = 4 (학습) / 8,12 (eval zero-shot)
    drone_id_emb: N_DRONE_MAX=12으로 확장

  [obs 슬라이스 (isaac_env_v12, 484차원)]
    [  0:300] DS map (E/F/U 10×10)
    [300:308] depth 8방향
    [308:310] self pos (x,y 0~1)
    [310:410] Fused Priority Map 100dim  (v12: other_vec 제거로 슬롯 앞당김)
    [410:482] active track K×6 = 72dim
    [482:484] battery (battery_norm, dist_to_base_norm)
    other_vec 완전 제거 → 드론 간 정보는 Entity Transformer attention으로 대체

  [scale-agnostic 설계]
    - TokenBuilder: N_D = all_obs.shape[1]로 실제 드론 수 동적 추출
    - RelationBiasV4: ND = pos_info.shape[1] - N_ZONE으로 동적
    - _build_mask, actor/critic forward 모두 가변 N_D
    - drone_id_emb(N_DRONE_MAX=12): 학습 4대, eval 8/12대 인덱스 모두 지원

  [TensorBoard 지표 확장]
    reward/*, loss/*, train/*           기존 유지
    perf/tracked_targets                신규 (tr_P < 0.3)
    perf/mean_trP                       신규
    perf/stale_count                    신규
    battery/mean_final                  신규
    battery/rtb_count                   신규
    battery/first_rtb_step              신규
    battery/active_at_end               신규
    tracking/r_track_mean               신규
    tracking/r_stale_mean               신규
    tracking/r_unc_mean                 신규
    tracking/trP_infantry               신규
    tracking/trP_tank                   신규
    tracking/trP_artillery              신규
    attn/drone_drone                    기존 유지
    attn/drone_zone                     기존 유지
    attn/drone_target                   신규
    attn/target_target                  신규
    attn/drone_to_infantry              신규
    attn/drone_to_tank                  신규
    attn/drone_to_artillery             신규
    token/zone_u_mean                   기존 유지
    token/drone_spread                  기존 유지
    token/target_trP_mean               신규
    token/target_age_mean               신규
"""
...
# v12: other_vec 제거 → OBS_DIM 490→484
# DS(300)+depth(8)+self(2)+fused_priority(100)+active(72)+battery(2) = 484
OBS_DIM      = 484
ACTION_DIM   = 4
DS_N_RL      = 10
MAP_W        = 100.0
...
OBS_ACTIVE_START    = 410           # K*6 = 72dim              (v12: 416→410)
OBS_BATTERY_START   = 482           # 2dim                     (v12: 488→482)
...
class TrainConfig:
    ...
    save_dir         : str   = str(REPO_ROOT / "attn_mappo_v12" / "stage1")
    log_interval     : int   = 10
...
class CurriculumConfig:
    stage         : int = 1
    base_save_dir : str = str(REPO_ROOT / "attn_mappo_v12")
...
class EntityActorNet(_BaseEntityNet):
    ...
    def forward(self, df, zf, af, gf, pi, attn_mask=None, return_attn=False):
        ...
        N_D = df.shape[1]          # v12: 실제 드론 수 (가변)
        drone_out = out[:, :N_D, :]            # (B,N_D,D)
        return self.head(drone_out), w         # (B,N_D,4), w
...
class EntityCriticNet(_BaseEntityNet):
    ...
    def forward(self, df, zf, af, gf, pi, attn_mask=None, return_attn=False):
        toks = self._build_tokens(df, zf, af, gf, pi)
        out, w = self.encoder(toks, attn_mask=attn_mask, return_attn=return_attn)
        N_D    = df.shape[1]       # v12: 실제 드론 수
        idx_g  = N_D + N_ZONE + K_ACTIVE  # global 토큰 위치 (가변)
...
class EntityMAPPOActor(nn.Module):
    ...
    def forward(self, obs, ep_prog=0.5, return_attn=False):
        B = obs.shape[0]
        df, zf, af, gf, pi = self.tb(obs, ep_prog)
        zone_u = zf[:, :, 4]
        msk    = self._build_mask(pi, zone_u, af, B)
        return self.net(df, zf, af, gf, pi, attn_mask=msk, return_attn=return_attn)
...
class MAPPOAgent:
    def __init__(self, cfg: TrainConfig):
        ...
        print(f"  Tokens : Drone×{N_DRONE_TRAIN}(train)/up to {N_DRONE_MAX}(eval) "
              f"+ Zone×{N_ZONE} + Track×{K_ACTIVE} + Global×{N_GLOBAL}")

    @torch.no_grad()
    def get_action_and_value(self, obs, deterministic=False, return_attn=False):
        obs_t  = torch.FloatTensor(obs).to(self.dev).unsqueeze(0)
        n_drone = obs.shape[0]   # v12: 실제 드론 수 (4/8/12)
        logits, aw = self.actor(obs_t, self.ep_prog, return_attn=return_attn)
        ...
        v, cw = self.critic(obs_t, self.ep_prog, return_attn=return_attn)
    ...
    def update(self):
        ...
        for _ in range(cfg.ppo_epochs):
            for mb_obs, mb_act, mb_lp, mb_ret, mb_adv in \
                ...
                # Critic update
                V, _ = self.critic(mb_obs, self.ep_prog)
                ...
                # Actor update
                logits, _ = self.actor(mb_obs, self.ep_prog)
    ...
    def save(self, ep):
        ...
        p = os.path.join(self.cfg.save_dir, f"mappo_v12_ep{ep:05d}.pt")
...
# 유틸
# ============================================================
def _patch_env_rewards(env, rd):
    import isaac_env_v12 as m
    for k, v in rd.items():
...
def _get_latest_checkpoint(directory):
    ...
    files = [f for f in os.listdir(directory)
             if f.startswith("mappo_v12_ep") and f.endswith(".pt")]
...
def train_stage(stage, cur_cfg=None, train_cfg=None):
    ...
    from isaac_env_v12 import DroneSwarmEnv
    print("\n[환경 초기화]")
    env = DroneSwarmEnv(randomize_terrain=False, terrain_seed=42)
    env.world.reset()
    for _ in range(50): env.world.step(render=False)

    rd = cur_cfg.stage_rewards.get(stage, {})
    if rd:
        print(f"\n[Stage {stage} 보상 오버라이드]")
        _patch_env_rewards(env, rd)
        for k, v in rd.items(): print(f"  {k:36s} = {v}")

    # 타겟 타입별 인덱스 (TensorBoard tracking by type용)
    target_types = env.target_types  # tuple of TargetType int
    from isaac_env_v12 import TargetType
    TGT_INFANTRY_IDX  = [k for k, t in enumerate(target_types) if t == TargetType.INFANTRY]
    TGT_TANK_IDX      = [k for k, t in enumerate(target_types) if t == TargetType.TANK]
    TGT_ARTILLERY_IDX = [k for k, t in enumerate(target_types) if t == TargetType.ARTILLERY]
    ...
    # non-terrain 셀 수 (uncertainty % 계산용)
    nt_non_terrain = int((~env.ds_map.terrain_mask).sum())
    monitor = StageMonitor(goal) if goal else None

    for ep in range(start+1, train_cfg.total_episodes+1):
        agent.episode  = ep
        ...
        for step in range(train_cfg.rollout_steps):
            ...
            acts, lps, vals, _, _ = agent.get_action_and_value(obs)
            nobs, rews, dones, info = env.step(acts.cpu().numpy())
            acc.update(info, step)
            last_info = info
            agent.buffer.push(obs_t, acts, lps,
        ...
        # GAE + update
        with torch.no_grad():
            _, _, nv, _, _ = agent.get_action_and_value(obs)
        ...
        # 타입별 tr(P) 로그
        if env.belief_tracker is not None:
            from isaac_env_v12 import TargetType, TargetState, T_STALE
            def mean_trP_for(idxs):
                return float(np.mean([
                    env.belief_tracker.get_tr_P_norm(k) for k in idxs]))
            logger.log_tracking_by_type(
                ep,
                mean_trP_for(TGT_INFANTRY_IDX),
                mean_trP_for(TGT_TANK_IDX),
                mean_trP_for(TGT_ARTILLERY_IDX))

            # v2: 상태별 타겟 지표 (TargetState enum 기반)
            states = env.belief_tracker.target_states
            committed_count = int(np.sum(states == TargetState.COMMITTED))
            stale_count     = int(np.sum(states == TargetState.STALE))
            candidate_count = int(np.sum(states == TargetState.CANDIDATE))

            committed_or_stale = (states == TargetState.COMMITTED) | (states == TargetState.STALE)
            if committed_or_stale.any():
                trP_committed = float(np.mean([
                    env.belief_tracker.get_tr_P_norm(k)
                    for k in range(env.num_targets)
                    if committed_or_stale[k]]))
            else:
                trP_committed = 1.0

            r_reacquire_ep = ep_info.get("r_reacquire_ep", 0.0)
            logger.log_committed(ep, committed_count, trP_committed,
                                  stale_count, candidate_count, r_reacquire_ep)

        # RTB 첫 발동 스텝 로그
        if ep_info["first_rtb_step"] >= 0:
            logger.log_battery_detail(ep, ep_info["first_rtb_step"])

        # 콘솔 출력
        if ep % train_cfg.log_interval == 0:
            logger.print_progress(ep, train_cfg.total_episodes,
                                   float(ep_r.mean()),
                                   ep_info, loss_info, elapsed)
            print()

        # attention 로그 (50ep마다)
        if ep % train_cfg.attn_log_interval == 0:
            with torch.no_grad():
                obs_t = torch.FloatTensor(obs).to(agent.dev).unsqueeze(0)
                _, aw  = agent.actor( obs_t, agent.ep_prog, return_attn=True)
                _, cw  = agent.critic(obs_t, agent.ep_prog, return_attn=True)
                df, zf, tf, gf, pi_pos = agent.actor.tb(obs_t, agent.ep_prog)
                # token 통계
                zone_u_mean   = float(zf[:,:,4].mean().cpu())
                # pi_pos: (B, N_DRONE+N_ZONE, 2) — drone 위치는 앞 N_DRONE개
                nd_actual     = pi_pos.shape[1] - N_ZONE
                ctr           = pi_pos[:,:nd_actual,:].mean(1, keepdim=True)
                drone_spread  = float(
                    torch.norm(pi_pos[:,:nd_actual,:]-ctr, dim=-1).mean().cpu())
                target_trP_mean = float(tf[:,:,4].mean().cpu())
                target_age_mean = float(tf[:,:,5].mean().cpu())
        ...
        # stage 달성 모니터
        if monitor:
            mv = (ep_info["uncertainty_pct"] if goal["metric"]=="uncertainty_pct"
                  else ep_info["terrain_hits_ep"])
            if monitor.update(mv):
                agent.save(ep)
                print(f"\n  Stage{stage} 조기 완료! (ep{ep})"); break

    agent.save(train_cfg.total_episodes
               if not (monitor and monitor.achieved) else ep)
