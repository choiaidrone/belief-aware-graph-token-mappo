...
============================
[Ablation: w/o IMM-KF / Active Track]
graph_token_mappo_v13.py 에서 IMM-KF 기반 target belief 입력을
모두 제거한 ablation variant.

제거:
  - fused priority map (obs[310:410]) → DS U map으로 대체
  - active track token (obs[410:482]) → neutral value로 대체
    (trP_norm=1.0: target confidence 없음, 나머지 0)

유지: DS map / Depth / Self / Graph branch (FOV 직접 관측만)

논문 비교 목적:
  Full Proposed vs w/o IMM-KF / Active Track
  → persistent tracking에 IMM-KF target belief가 얼마나 중요한지 확인.
  → 가장 중요한 ablation 실험.

주의: 환경 내부 tracker는 그대로 유지 (reward/metric 공정성).
      policy observation에서만 IMM-KF-derived input을 제거.

저장 경로: <repository root>/ablation_wo_track_v13/
"""
# ──────────────────────────────────────────────────────────────────────
# 원본: graph_token_mappo_v13.py
"""
========================
...
class TrainConfig:
    ...
    save_dir         : str   = str(REPO_ROOT / "ablation_wo_track_v13" / "stage1")
    log_interval     : int   = 10
...
class CurriculumConfig:
    stage         : int = 1
    base_save_dir : str = str(REPO_ROOT / "ablation_wo_track_v13")
...
class GraphTokenObsWrapper:
    ...
    [Ablation w/o IMM-KF / Active Track]
    1) fused priority slot [310:410] → DS U map으로 대체
    2) active track slot [410:482]   → neutral value로 대체
       (trP_norm=1.0, 나머지 0: IMM-KF belief 없음을 명시)
    obs dim 484 그대로 유지 — 네트워크 구조 변경 없음.
    ...
    # ── ablation 함수 ─────────────────────────────────────────────────
    @staticmethod
    def _ablate_target_belief_inputs(obs484: np.ndarray) -> np.ndarray:
        """
        w/o IMM-KF / Active Track:
          1. fused priority [310:410] → DS U map (belief fusion 제거)
          2. active track  [410:482] → neutral value
               trP_norm(feat idx 4) = 1.0  (confidence 없음)
               나머지 feat           = 0.0
        환경 내부 tracker는 건드리지 않음 (reward/metric 공정성 유지).
        """
        obs484 = obs484.copy()

        # 1. fused priority → U map으로 대체
        u_pool = obs484[:, OBS_U_START:OBS_U_START + 100]
        obs484[:, OBS_INTENSITY_START:OBS_INTENSITY_START + 100] = u_pool

        # 2. active track → neutral (target belief 완전 차단)
        n_agents = obs484.shape[0]
        active = obs484[:, OBS_ACTIVE_START:OBS_ACTIVE_START + K_ACTIVE * ACTIVE_FEAT_DIM]
        active = active.reshape(n_agents, K_ACTIVE, ACTIVE_FEAT_DIM).copy()
        active[:] = 0.0
        active[:, :, 4] = 1.0   # trP_norm = 1.0 (uncertainty 최대, confidence 없음)
        active[:, :, 5] = 0.0   # age_norm = 0.0
        obs484[:, OBS_ACTIVE_START:OBS_ACTIVE_START + K_ACTIVE * ACTIVE_FEAT_DIM] = \
            active.reshape(n_agents, K_ACTIVE * ACTIVE_FEAT_DIM)

        return obs484

    def reset(self):
        obs484 = self.env.reset()
        obs484 = self._ablate_target_belief_inputs(obs484)          # ← ablation
...
class MAPPOAgent:
    ...
    def save(self, ep):
        ...
        p = os.path.join(self.cfg.save_dir, f"wo_track_ep{ep:05d}.pt")
...
def _get_latest_checkpoint(directory):
    ...
    files = [f for f in os.listdir(directory)
             if f.startswith("wo_track_ep") and f.endswith(".pt")]
