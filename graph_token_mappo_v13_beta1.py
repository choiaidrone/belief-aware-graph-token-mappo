...
class TrainConfig:
    ...
    save_dir         : str   = str(REPO_ROOT / "graph_token_beta1_v13" / "stage1")
    log_interval     : int   = 10
...
class CurriculumConfig:
    stage         : int = 1
    base_save_dir : str = str(REPO_ROOT / "graph_token_beta1_v13")
...
class MAPPOAgent:
    ...
    def save(self, ep):
        ...
        p = os.path.join(self.cfg.save_dir, f"beta1_ep{ep:05d}.pt")
...
def _get_latest_checkpoint(directory):
    ...
    files = [f for f in os.listdir(directory)
             if f.startswith("beta1_ep") and f.endswith(".pt")]
...
def train_stage(stage, cur_cfg=None, train_cfg=None):
    ...
    from isaac_env_v13_beta1 import DroneSwarmEnv
    print("\n[환경 초기화]")
    ...
    for ep in ep_bar:
        ...
        if getattr(base_env, "belief_tracker", None) is not None:
            ...
            from isaac_env_v13_beta1 import TargetState
