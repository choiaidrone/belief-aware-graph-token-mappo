...
============================
[Ablation: w/o Fused Priority]
graph_token_mappo_v13.py 에서 fused priority map(obs[310:410])을
DS uncertainty pool(U map)로 대체한 ablation variant.

제거: target-belief fusion (target intensity × IMM state → priority map)
유지: DS map / Depth / Self / IMM-KF active-track token / Graph branch

논문 비교 목적:
  Full Proposed vs w/o Fused Priority
  → uncertainty map과 target belief를 결합한 spatial priority가
    추적 성능(AUC tr(P), committed ratio)에 기여하는지 확인.

저장 경로: <repository root>/ablation_wo_fused_v13/
"""
# ──────────────────────────────────────────────────────────────────────
...
class TrainConfig:
    ...
    save_dir         : str   = str(REPO_ROOT / "ablation_wo_fused_v13" / "stage1")
    log_interval     : int   = 10
...
class CurriculumConfig:
    stage         : int = 1
    base_save_dir : str = str(REPO_ROOT / "ablation_wo_fused_v13")
...
class GraphTokenObsWrapper:
    ...
    [Ablation w/o Fused Priority]
    fused priority slot [310:410]을 DS U map으로 대체.
    ...
    # ── ablation 함수 ─────────────────────────────────────────────────
    @staticmethod
    def _ablate_fused_to_u_only(obs484: np.ndarray) -> np.ndarray:
        """
        fused priority slot [310:410]을 DS U map(uncertainty pool)으로 대체.
        target-belief fusion 정보만 제거하고 나머지는 그대로 유지.
        """
        obs484 = obs484.copy()
        u_pool = obs484[:, OBS_U_START:OBS_U_START + 100]          # DS U map (100dim)
        obs484[:, OBS_INTENSITY_START:OBS_INTENSITY_START + 100] = u_pool
        return obs484

    def reset(self):
        obs484 = self.env.reset()
        obs484 = self._ablate_fused_to_u_only(obs484)              # ← ablation
...
class MAPPOAgent:
    ...
    def save(self, ep):
        ...
        p = os.path.join(self.cfg.save_dir, f"wo_fused_ep{ep:05d}.pt")
...
def _get_latest_checkpoint(directory):
    ...
    files = [f for f in os.listdir(directory)
             if f.startswith("wo_fused_ep") and f.endswith(".pt")]
