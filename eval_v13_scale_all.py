"""
eval_v13_scale_all.py — Scale Experiment: GAT / CC-MPSO / SSM / Random / Graph-Token
======================================================================================
[실험 설정]  Fixed Ratio 1:5 (Zero-shot scalability)
    4 drones  / 20 targets  ← 학습 환경과 동일
    8 drones  / 40 targets  ← 4드론 학습 모델로 제로샷
    12 drones / 60 targets  ← 4드론 학습 모델로 제로샷

[환경]   isaac_env_v13.py  (pure NumPy — Isaac/omni 불필요)
         v13 수정사항: num_anti_air 가변화 → 4x20/8x40/12x60 그대로 사용 가능
...
[Safety / Coordination 지표 — 교수님 피드백 반영]
    Drone collisions → Inter-UAV proximity events (물리 충돌 X, 안전거리 위반 이벤트)
    섹션: Safety / Coordination
    Main metrics   : uncertainty_pct, unc_reduction_pct, ext_auc_mean_trP,
                     committed_ratio, stale_ratio_norm
    Safety / Coord : drone_prox_events (↓), active_at_end (↑)
    보조 지표       : battery_depletion_count (↓)

[CSV 출력]
    <repository root>/eval_results/v13/
        scale_4x20_gat.csv  scale_4x20_ccmpso.csv  ...  (총 15개)
    컬럼에서 drone_colls → drone_prox_events 로 이름 변경
...
[실행법]
    ...
    # 특정 method/scale만
    python eval_v13_scale_all.py --n_eval 100 --methods gat ssm --scales 4x20

    # 처음부터 다시
    python eval_v13_scale_all.py --n_eval 100 --restart ^
        --ckpt_gat        "..." ^
        --ckpt_graph_token "..."
"""
...
OUT_DIR   = str(REPO_ROOT / "eval_results" / "v13")
MAX_STEPS = 3000
...
def main():
    parser = argparse.ArgumentParser(
        description="Scale Experiment v13 — GAT / CC-MPSO / SSM / Random / Graph-Token"
    )
    ...
    parser.add_argument("--ckpt_gat", type=str, default=None,
                        help="GAT-MAPPO 체크포인트 .pt 경로 (3 scale 공통)")
    parser.add_argument("--ckpt_graph_token", type=str, default=None,
                        help="Graph-Token MAPPO 체크포인트 .pt 경로 (3 scale 공통)")
    ...
    print(f"\n{'='*72}")
    print(f"  eval_v13_scale_all.py  —  Scale Experiment v13 (Zero-shot)")
    ...
    print(f"  Safety지표: drone_colls → drone_prox_events (Inter-UAV proximity)")
    ...
    for scale_key in args.scales:
        ...
        for method in args.methods:
            combo_idx += 1
            csv_name = f"scale_{scale_key}_{method}.csv"
    ...
    print(f"\n{'='*72}")
    print(f"  eval_v13_scale_all.py 완료  →  {OUT_DIR}")
    print(f"{'='*72}\n")
